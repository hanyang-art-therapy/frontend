...
export default function NoticeDetail({ data }: NoticeDetailProps) {
  ...
  if (!notice)
    return (
      <div className='min-h-screen flex flex-col items-center justify-start text-start text-black mt-[120px]'>
        {/* 제목 */}
        <div className='w-full max-w-[1260px] h-[600px] xl:px-0 border-t-2 border-b-2 py-[10px] border-t-gray-300 border-b-gray-300 bg-white flex justify-center items-center'>
  ...
  return (
    <div className='flex flex-col items-center justify-start text-start text-black mt-[80px] md:mt-[120px] w-full h-full'>
      {/* 제목 */}
      <div
        className='w-full max-w-[1260px] md:h-[140px] xl:px-0 border-t-2 py-[10px] text-start'
        style={{ backgroundColor: 'rgba(221, 221, 221, 0.2)' }}
      >
        <div className='flex flex-col gap-4 mt-2 md:text-sm px-[20px]'>
          <h1 className='text-[18px] md:text-4xl font-bold'>{notice.title}</h1>
          <div className='md:flex md:flex-row t-r-14 md:p-[10px] flex gap-2 md:gap-4'>
            <div className='flex items-center gap-2'>
              <strong className='text-[#666]'>구분</strong>
              {notice.category}
            </div>
            <div className='flex items-center gap-2'>
              <strong className='text-[#666]'>작성일</strong>

              {new Date(notice.createdAt).toLocaleDateString('ko-KR')}
            </div>
            <div className='flex items-center gap-2'>
              <strong className='text-[#666]'>기간</strong>
              {notice.periodStart
            ...
            <div className='flex items-center gap-2'>
              <strong className='text-[#666]'>조회수</strong>
      ...
      {/* 본문 내용 */}
      <div className='w-full max-w-[1260px] h-auto p-[20px] md:p-[30px]'>
        {/* <h2 className='text-[20px] font-semibold leading-relaxed py-4'>
          {notice.title}
        </h2> */}
        <div className='mt-2 text-[14px] md:text-[18px] leading-relaxed'>
      ...
      {/* 파일 */}
      <div
        className='w-full max-w-[1260px] h-auto  md:px-5 py-4 md:py-6 border-t flex flex-col gap-2'
        style={{ backgroundColor: 'rgba(221, 221, 221, 0.2)' }}
      >
      ...
      {/* 이전글과 다음글 */}
      <div className='w-full max-w-[1260px] px-5 xl:px-0 py-6 border-t text-[16px] flex justify-center'>
        <NoticeNav noticeNo={noticeNo ?? ''} />
      </div>
    </div>
  );
