  ...
  return (
    <div className='space-y-[50px]'>
      ...
      <section className='space-y-[30px]'>
        ...
        <div className='space-y-2.5 text-r-18'>
          <p>{section1.paragraphs[0]}</p>

          <p>{section1.paragraphs[1]}</p>
        </div>

        <div className='space-y-2.5 text-r-14 text-gray'>
      ...
      <section className='space-y-[30px]'>
        <div className='space-y-2.5'>
          <h3 className='title-b-18'>{section2.privacyTitle}</h3>

          <p className='text-r-14'>{section2.privacyDescription}</p>
        </div>

        {/* 개인정보 수집 이용 안내 테이블 */}
        <div className='space-y-2.5'>
          <h3 className='title-m-18'>{section2.collectPrivacyTitle}</h3>

          <div className='border border-bg-gray overflow-hidden'>
            <table className='w-full'>
              <thead>
                <tr className='flex h-[45px] items-center text-r-16 bg-bg-muted border-b border-bg-gray'>
                  {section2.collectPrivacyItems.map((text, idx) => (
                    <td
                      key={idx}
                      className={cn(
                        'w-1/3 h-full flex items-center justify-center text-center',
                        idx < 2 && 'border-r border-bg-gray'
                      )}>
                ...
                <tr className='flex'>
                  {section2.collectPrivacyDetails.map((detail, idx) => (
                    <td
                      key={idx}
                      className={cn(
                        'w-1/3 h-32 border-r border-bg-gray p-2 overflow-y-auto',
                        idx === 2 && 'border-r-0'
                      )}>
                      ...
                      <ul className='space-y-1 mt-2'>
                        {detail.list.map((item, idx) => (
                          <li key={idx} className='text-r-14'>
                            · {item}
                          </li>
                        ))}
                      </ul>
        ...
        <div className='space-y-2 mt-6'>
          <h3 className='title-b-18'>{section2.rejectTitle}</h3>
          <span>
            <p className='text-r-16'>{section2.rejectDescription1}</p>
            <p className='text-r-16'>{section2.rejectDescription2}</p>
          </span>
        </div>
        ...
        <RadioGroup
          ...
          <div className='flex items-center space-x-1.5'>
            <label htmlFor='disagree' className='title-b-16'>
              동의하지 않음
            </label>
